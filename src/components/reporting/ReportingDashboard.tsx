...
const ReportingDashboard: React.FC<ReportingDashboardProps> = ({ selectedClient }) => {
  ...
  // Get client name based on selectedClient
  const getClientName = () => {
    if (selectedClient === 'company-alpha') {
      return 'Company Alpha';
    } else if (selectedClient === 'acme-industries') {
      return 'Acme Industries';
    } else if (selectedClient === 'western-manufacturing') {
      return 'Western Manufacturing';
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 justify-between items-center">
        <div className="space-y-2">
          <h2 className="text-xl font-semibold">{getClientName()} - Gas Reports</h2>
      ...
      <div className="space-y-6">
        <h3 className="text-lg font-medium">
          {reportType === 'consumption' && `${getClientName()} - Gas Consumption Report`}
          {reportType === 'delivery' && `${getClientName()} - Gas Delivery Report`}
          {reportType === 'efficiency' && `${getClientName()} - Gas Efficiency Report`}
        </h3>
